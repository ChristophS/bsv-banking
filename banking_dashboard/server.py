        ...
        if hide_completed_vorgaenge:
            conditions.append(
                ...
                NOT (
                    EXISTS (
                        SELECT 1
                        FROM transaktion_vorgaenge AS completed_tv
                        WHERE completed_tv.transaktions_id =
                              n.transaktions_id
                    )
                    AND NOT EXISTS (
                        SELECT 1
...
class DashboardRequestHandler(BaseHTTPRequestHandler):
    ...
    def _transactions_response(self, query: dict[str, list[str]]) -> None:
        ...
        hide_completed_vorgaenge = _truthy_query_value(
            query.get("hide_completed_vorgaenge", ["false"])[0]
        )
