...
class DashboardHTTPTests(unittest.TestCase):
    ...
    def test_mail_import_without_transaction_links_stays_successful_over_http(self):
        with urlopen(self.base_url + "/api/mail", timeout=5) as response:
            inbox_id = json.load(response)["messages"][0]["id"]
        ...
        vorgang = imported["vorgang"]
        self.assertEqual(1, len(vorgang["mails"]))
        self.assertEqual(inbox_id, vorgang["mails"][0]["inbox_id"])
        self.assertEqual([], vorgang["transaktionen"])

    def test_mail_import_rejects_unknown_transaction_link_over_http(self):
        with urlopen(self.base_url + "/api/mail", timeout=5) as response:
            inbox_id = json.load(response)["messages"][0]["id"]
            ...
            data=json.dumps(
                {
                    ...
                    "todos": [],
                    "termine": [],
                    "links": {"transaction_ids": ["tx_unknown"]},
                }
            ).encode("utf-8"),
            headers={"Content-Type": "application/json"},
            method="POST",
        ...
        self.assertIn("tx_unknown", payload["error"])
